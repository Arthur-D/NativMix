...
APP_VERSION="1.0.6"
APP_RELEASE="1"
SPEC_FILE="packaging/fedora/nativmix.spec"
...
echo "Starting automated RPM build for NativMix v${APP_VERSION}-${APP_RELEASE} (Fedora)..."
...
if [ $? -eq 0 ]; then
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        sudo dnf install --nogpgcheck "${RPM_FILE}"
    fi
else
